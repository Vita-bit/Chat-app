...
def recv_json(sock: socket.socket) -> dict | None:
    """
    Receive a length-prefixed JSON message from *sock* and return it as a dictionary.
    """
    try:
...
def create_chat(users: list, creator: str, conn: socket.socket, chat_name: str = None, avatar: str = None):
    """
    Create a new chat in the database and notify all members via their active connections.
    """
...
def get_chats(username: str):
    """
    Query all chats the given user belongs to and send the list to that user's socket.
    """
    with sqlite3.connect("chatapp.db") as dbconn:
        dbconn.execute("PRAGMA foreign_keys = ON")
        cur = dbconn.cursor()
...
def open_chat(chat_id: int, username: str):
    """
    Validate the user's membership in the chat, load the last 50 messages,
    and send them to the user's socket. Also records the user's active chat.
    """
    with sqlite3.connect("chatapp.db") as dbconn:
        cur = dbconn.cursor()
        cur.execute("select id from users where username = ?", (username,))
...
def get_users(requesting_user: str):
    """
    Retrieve all registered usernames (excluding the requester) and send them to the requester's socket.
    """
    with sqlite3.connect("chatapp.db") as dbconn:
        cur = dbconn.cursor()
...
def msg(user: str, content: str):
    """
    Store a text message from *user* in the database and broadcast it to all members
    of the user's currently active chat.
    """
    with current_chats_lock:
...
def sql_init() -> bool:
    """
    Initialise the SQLite database by creating all required tables if they do not already exist.
    """
    try:
        with sqlite3.connect("chatapp.db") as dbconn:
            dbconn.execute("PRAGMA foreign_keys = ON")
...
def handle_client(conn: socket.socket, addr: tuple):
    """
    Handle all communication with a single connected client for its entire session.

    Manages authentication (login/register) on the first message, then dispatches
    subsequent messages to the appropriate handler functions. Cleans up the client
    entry on disconnection.
    """
    username = None
    try:
        while True:
